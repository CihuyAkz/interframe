...
public final class FrameGenerator {
    ...
    // Ping-pong capture: textures[idx] holds the newest frame, textures[1-idx] the previous one.
    ...
    public void onPresent() {
        ...
        try {
            ...
            // Advance ping-pong and capture frame N into the new "current" slot.
            idx = 1 - idx;
    ...
    private boolean generate(InterframeConfig cfg, int w, int h, int prevIdx, int currIdx) {
        ...
        ctx.width = w;
        ctx.height = h;
        ctx.prevColorTex = colorTex[prevIdx];
        ctx.currColorTex = colorTex[currIdx];
